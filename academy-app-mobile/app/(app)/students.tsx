...
import { formatPrice, formatShortDate } from '@/utils/general';
...
export default function Students() {
  ...
  const { data, isLoading, isError } = useQuery<{ students: Student[] }>({
    queryKey: ['students'],
    queryFn: () => api<{ students: Student[] }>('/api/v1/students'),
  });
  ...
  return (
    <SafeAreaView className="flex-1 bg-background" edges={['top']}>
      <ScrollView contentContainerClassName="px-4 py-6 gap-4">
        ...
        {students.length === 0 ? (
          <EmptyState message={search ? 'Sin resultados para esa búsqueda.' : 'No hay alumnos registrados.'} emoji="👥" />
        ) : (
