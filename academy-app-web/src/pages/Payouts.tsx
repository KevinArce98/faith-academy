...
export default function Payouts() {
	...
	const { data, isLoading, isError } = useQuery<PayoutsResponse>({
		...
		queryFn: () =>
			apiClient<PayoutsResponse>(`/api/v1/payouts?period=${period}`),
	});
			...
			{isLoading ? (
				...
				<>
					...
					<div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
						...
						<StatCard
							label="Por cobrar"
							value={formatPrice(data.totals.pending)}
							tone="warning"
						/>
						<StatCard
							label="Repartido a profes"
							value={formatPrice(data.totals.allocated)}
						/>
						<StatCard
					...
					{data.payouts.length === 0 ? (
						<div className="rounded-xl border border-dashed border-gray-200 py-16 text-center text-sm text-gray-400">
							No hay pagos que repartir este mes. Registra mensualidades pagadas
							e inscripciones para ver el cálculo.
						</div>
					) : (
										...
										<div className="flex items-center gap-3">
											...
											<div>
												<p className="text-dark font-bold">
													{t.teacherName || 'Sin profesor'}
												</p>
												<p className="text-xs text-gray-400">
													{t.classes.length}{' '}
													{t.classes.length === 1 ? 'clase' : 'clases'}
													{t.hoursWorked > 0 &&
														` · ${t.hoursWorked}h trabajadas`}
												</p>
											</div>
										</div>
