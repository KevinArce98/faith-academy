...
export default function Scanner() {
  ...
  useEffect(() => {
    ...
    reader
      .decodeFromVideoDevice(undefined, videoRef.current, async (result, error) => {
        ...
        setLoading(true);
        try {
          const data = await apiClient<Omit<ScanResult, 'scannedAt'>>('/api/v1/attendance/scan', {
            method: 'POST',
            body: JSON.stringify({ token }),
          });
          setHistory((prev) => [{ ...data, scannedAt: new Date() }, ...prev].slice(0, 20));
        } catch {
          setHistory((prev) =>
  ...
  function getCardStyle(entry: ScanResult): React.CSSProperties {
    if (!entry.ok)
      return {
        ...
        borderColor: studioConfig.colors.danger,
      };
    if (entry.nearExpiry)
      return {
        backgroundColor: 'var(--color-warning-bg)',
        borderColor: studioConfig.colors.warning,
      };
    return {
      backgroundColor: 'var(--color-success-bg)',
      borderColor: studioConfig.colors.success,
    };
  }
  ...
  return (
    <div
      ...
      style={{ backgroundColor: studioConfig.colors.background }}
    >
      {/* Header */}
      <header
      ...
      >
        ...
        <h1 className="text-xl font-semibold tracking-tight text-white">Scanner de Asistencia</h1>
      </header>
      ...
      <div className="flex flex-1 flex-col gap-6 p-6 lg:flex-row">
        ...
        <div className="flex flex-1 flex-col items-center gap-4">
          <div
            ...
            style={{
              borderColor: scanning ? PRIMARY : 'var(--color-gray-400)',
            }}
          >
        ...
        <div className="flex w-full flex-col gap-3 lg:w-80">
          ...
          {history.length === 0 && <p className="text-sm text-slate-400">Sin escaneos aún.</p>}
                ...
                {entry.ok && (
                  <>
                    <span
                      ...
                      style={{
                        color: entry.nearExpiry
                          ? studioConfig.colors.warning
                          : studioConfig.colors.success,
                      }}
                    >
