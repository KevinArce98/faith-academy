...
export default function ClassAttendance() {
	...
	const classes = useMemo(() => {
		...
		const mine =
			role === 'TEACHER' ? all.filter((c) => c.teacherId === userId) : all;
		return mine.filter((c) =>
			c.oneOffDate
				? c.oneOffDate === date
				: c.slots?.some((s) => s.dayOfWeek === dow),
		);
		...
		<div className="space-y-6">
			...
			<div className="flex flex-col gap-3 sm:flex-row sm:items-end">
				...
				<SelectMenu
					...
					onChange={setClassId}
					placeholder={
						classes.length === 0 ? 'Sin clases este día' : 'Seleccionar clase'
					}
					...
					{roster.map((s) => {
						...
						return (
							<button
								...
								title={
									present
										? 'Clic para quitar la asistencia'
