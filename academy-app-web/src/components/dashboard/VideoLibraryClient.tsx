...
import {
  Search, ChevronDown, Pencil, Trash2, PlayCircle, FileText, Upload,
} from 'lucide-react';
...
export function VideoLibraryClient({ contents }: VideoLibraryClientProps) {
  ...
  const tags = [
    'todo',
    ...Array.from(
      new Set(contents.map((c) => c.class?.name ?? 'Material Global'))
    ),
  ];
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-center">
        ...
        <div className="flex items-center gap-3">
          <Button variant="text" color="neutral" className="h-9 shrink-0 px-3 text-sm border border-gray-200 hover:bg-gray-50 gap-1.5">
      ...
      {/* Tag pills */}
      <div className="flex gap-2 overflow-x-auto pb-1 md:flex-wrap" style={{ WebkitOverflowScrolling: 'touch' } as React.CSSProperties}>
        {tags.map((tag) => (
          <Button
            key={tag}
          ...
          return (
            ...
            >
              <div className="bg-dark relative flex h-36 items-center justify-center">
                ...
                <span className={cn('absolute top-2 left-2 rounded-full px-2 py-0.5 text-[10px] font-bold', getTagColor(tagName))}>
                  {tagName}
                </span>
              </div>
