    ...
    <div className="space-y-8">
      ...
      {planExpired && (
        <div className="border-danger/20 bg-danger/5 flex flex-col gap-2 rounded-2xl border px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="text-danger flex items-start gap-2 text-sm font-medium">
            ...
            <span>
              Tu plan venció el {formatExpiry(expiresAt)}. Renueva tu mensualidad para seguir
              inscribiéndote en clases.
            </span>
          </div>
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-3 md:gap-5">
        ...
        <div className="rounded-2xl border border-gray-50 bg-white p-4 shadow-sm md:p-5">
          ...
          {subscription ? (
            <>
              <p className="text-dark mb-1 text-lg font-bold md:text-xl">{subscription.planName}</p>
            ...
            <>
              <p className="mb-1 text-lg font-bold text-gray-300 md:text-xl">—</p>
              <Link to="/plans" className="text-primary text-xs font-semibold hover:underline">
                Ver planes →
              </Link>
            </>
          )}
        ...
        <div className="rounded-2xl border border-gray-50 bg-white p-4 shadow-sm md:p-5">
          ...
          <p
            className={cn(
              ...
              planActive ? 'text-success' : planExpired ? 'text-danger' : 'text-warning'
            )}
          >
            {planActive ? 'Activo' : planExpired ? 'Vencido' : 'Pendiente'}
          </p>
          {planActive && expiresAt ? (
            <p className="text-xs font-medium text-gray-400">Vence el {formatExpiry(expiresAt)}</p>
          ) : enrollmentFee != null ? (
            <p className="text-xs font-medium text-gray-400">
        ...
        <div className="col-span-2 rounded-2xl border border-gray-50 bg-white p-4 shadow-sm md:col-span-1 md:p-5">
          ...
          <p className="text-dark mb-1 text-2xl font-bold md:text-4xl">{classesThisMonth.length}</p>
      ...
      <div className="rounded-2xl border border-gray-50 bg-white p-4 shadow-sm md:p-6">
        <h2 className="text-dark mb-4 text-base font-bold">Mis Clases este Mes</h2>
