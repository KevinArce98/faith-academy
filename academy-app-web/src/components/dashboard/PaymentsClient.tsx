...
export function PaymentsClient({
  ...
  const uniquePlans = Array.from(new Map(orders.map((o) => [o.plan.id, o.plan])).values()).sort(
  ...
  const tabCount = (key: TabKey) =>
    key === 'ALL'
      ? orders.filter(
          (o) =>
            matchesDateRange(o.createdAt, dateRange) &&
            (planFilter === 'all' || o.plan.id === planFilter)
        ).length
      : orders.filter(
          (o) =>
            o.status === key &&
  ...
  const selectedDateLabel = DATE_RANGE_OPTIONS.find((o) => o.key === dateRange)?.label ?? 'Fecha';
      ...
      {isAdmin && (
        <div>
          ...
          <div
            className={cn(
              'items-center gap-3',
              filtersOpen ? 'mt-3 flex flex-wrap' : 'hidden md:flex'
            )}
          >
            {/* Date range dropdown */}
            <FilterDropdown
