...
type ConfirmState =
  { type: 'delete'; teacher: TeacherProfile } | { type: 'role'; teacher: TeacherProfile };
...
export function TeachersClient({ teachers, activeCount }: TeachersClientProps) {
  ...
  async function toggleActive(teacher: TeacherProfile) {
    ...
    try {
      ...
      setToast({
        type: 'error',
        message: getErrorMessage(err, 'Error al actualizar.'),
      });
    } finally {
      setLoadingId(null);
    }
  ...
  async function changeRoleToStudent(teacher: TeacherProfile) {
    ...
    try {
      ...
      setToast({
        type: 'success',
        message: `${teacher.name ?? 'Profesor'} ahora es alumno.`,
      });
      queryClient.invalidateQueries({ queryKey: ['teachers'] });
    } catch (err) {
      setToast({
        type: 'error',
        message: getErrorMessage(err, 'Error al cambiar el rol.'),
      });
    } finally {
      setLoadingId(null);
      setConfirm(null);
  ...
  async function deleteTeacher(teacher: TeacherProfile) {
    ...
    try {
      ...
      setToast({
        type: 'success',
        message: `${teacher.name ?? 'Profesor'} eliminado.`,
      });
      queryClient.invalidateQueries({ queryKey: ['teachers'] });
    } catch (err) {
      setToast({
        type: 'error',
        message: getErrorMessage(err, 'Error al eliminar.'),
      });
    } finally {
      setLoadingId(null);
      setConfirm(null);
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex flex-col items-end gap-2">
                    ...
                    <TeacherMenu
                      ...
                      onToggleMenu={() => setMenuOpen(menuOpen === teacher.id ? null : teacher.id)}
                      onEdit={() => {
                        setEditing(teacher);
                        setMenuOpen(null);
                      }}
