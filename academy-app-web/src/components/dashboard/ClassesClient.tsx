...
type ReserveState = { classId: string; status: 'loading' | 'success' | 'error'; message?: string } | null;
...
export function ClassesClient({
  ...
  async function handleReserve(cls: Cls) {
    ...
    try {
      ...
      setReserveState({ classId: cls.id, status: 'success', message: `Reservado en ${cls.name}` });
      setStudentModalClass(null);
      queryClient.invalidateQueries({ queryKey: ['classes'] });
      queryClient.invalidateQueries({ queryKey: ['dashboard'] });
      setTimeout(() => setReserveState(null), 3000);
    } catch (err) {
      const message = err instanceof Error ? err.message : 'No se pudo reservar la clase';
  ...
  function renderEventContent(arg: EventContentArg) {
    ...
    return (
      ...
      >
        ...
        <p className={cn('text-[10px]', isFull ? 'text-danger font-bold' : 'opacity-60')}>
          {cls._count.attendances}/{cls.maxCapacity}{isFull ? ' · Llena' : ''}
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Week nav */}
      <div className="flex items-center gap-3">
        <Button variant="text" color="neutral" onClick={prevWeek} className="h-auto p-1.5">
          <ChevronLeft className="h-4 w-4 text-gray-500" />
        </Button>
        <p className="text-dark text-sm font-semibold capitalize">{weekLabel}</p>
        <Button variant="text" color="neutral" onClick={nextWeek} className="h-auto p-1.5">
      ...
      {view === 'list' && (
        <>
          ...
          <div className="hidden md:block">
            <TableContainer>
              {classes.length === 0 ? (
                <p className="py-12 text-center text-sm text-gray-400">Sin clases esta semana</p>
              ) : (
                <Table>
                  <TableHead>
                  ...
                  <TableBody ref={listRef}>
                    {listPagination.paginated.map((cls) => {
                      const start = new Date(cls.startsAt);
                      const end = new Date(cls.endsAt);
                      const dayName = new Intl.DateTimeFormat('es-CR', { weekday: 'long' }).format(start);
                      const timeStr = `${formatTime(start)} — ${formatTime(end)}`;
                      const isFull = cls._count.attendances >= cls.maxCapacity;
                      const isReserving = reserveState?.classId === cls.id && reserveState.status === 'loading';
                      return (
                        <TableRow key={cls.id}>
                          <TableCell className="text-dark pl-5 font-medium">{cls.name}</TableCell>
                          <TableCell className="text-gray-600 capitalize">{dayName}</TableCell>
                          <TableCell className="font-mono text-xs text-gray-600">{timeStr}</TableCell>
                          <TableCell>
                            <span
                              className={cn(
                                'rounded-full border px-2 py-0.5 text-xs font-medium',
                                LEVEL_COLORS[cls.skillLevel] ?? 'border-transparent bg-gray-100 text-gray-600'
                              )}
                            >
                              {LEVEL_TRANSLATIONS[cls.skillLevel] || cls.skillLevel}
                            </span>
                          </TableCell>
                          <TableCell className={cn('text-gray-600', isFull && 'text-danger font-semibold')}>
          ...
          <div className="space-y-4 md:hidden">
            {classes.length === 0 ? (
              ...
              (() => {
                const grouped = listPagination.paginated.reduce<Record<string, typeof classes>>(
                  (acc, cls) => {
                        ...
                        {dayClasses.map((cls) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <p className={cn('mb-3 text-xs', isFull ? 'text-danger font-semibold' : 'text-gray-400')}>
      ...
      <AnimatePresence>
        ...
        {deleteModalOpen && selectedClass && isAdmin && (
          <DeleteClassModal
            ...
            classData={selectedClass}
            onClose={() => { setDeleteModalOpen(false); setSelectedClass(null); }}
          />
        )}
