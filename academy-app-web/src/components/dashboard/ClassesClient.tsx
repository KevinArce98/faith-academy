...
import { Clock, GraduationCap, Pencil, Plus, Search, Trash2 } from 'lucide-react';
...
export function ClassesClient({ classes, teachers, role }: ClassesClientProps) {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((cls) => (
            ...
            >
              ...
              {cls.description && (
                <p className="text-sm leading-snug text-gray-400">{cls.description}</p>
              )}
      ...
      <AnimatePresence>
        {creating && (
          <NewClassModal key="new-class" teachers={teachers} onClose={() => setCreating(false)} />
        )}
        {editing && (
          <EditClassModal
            key="edit-class"
