...
import { UserX, Users, X } from 'lucide-react';
...
import { formatTime, getInitials } from '@/utils/general';
...
export function ClassStudentsModal({ cls, onClose }: Props) {
	...
	const { data, isLoading } = useQuery({
		...
		queryFn: () =>
			apiClient<{ attendances: EnrolledStudent[] }>(
				`/api/v1/classes/${cls.id}/attendances`,
			),
	});

	const removeMutation = useMutation({
		mutationFn: (attendanceId: string) =>
			apiClient(`/api/v1/classes/${cls.id}/attendances/${attendanceId}`, {
				method: 'DELETE',
			}),
		onSuccess: () => {
