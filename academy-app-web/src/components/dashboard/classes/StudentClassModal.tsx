import { X, BookCheck, Clock, Users, BarChart2 } from 'lucide-react';
    ...
    >
      ...
      >
        ...
        <div className="mb-4 flex items-start justify-between gap-3">
          <h2 className="text-dark text-lg font-bold leading-tight">{cls.name}</h2>
          <button
            onClick={onClose}
        ...
        <div className="mb-5 space-y-3">
          ...
          <div className="flex items-center gap-2.5 text-sm">
            ...
            <span className={cn(isFull ? 'text-danger font-semibold' : 'text-gray-600')}>
              {cls._count.attendances}/{cls.maxCapacity} inscritos
              {isFull ? ' · Clase llena' : ` · ${spotsLeft} cupo${spotsLeft !== 1 ? 's' : ''} disponible${spotsLeft !== 1 ? 's' : ''}`}
            </span>
          </div>
          {cls.description && (
