...
const conditionalNavItems: Array<NavItem & { feature: keyof typeof studioConfig.features }> = [
  {
    href: '/video-library',
    icon: PlayCircle,
...
export function Sidebar({ user, onClose }: SidebarProps) {
  ...
  const navItems: NavItem[] = [
    ...
    ...conditionalNavItems.filter((item) => studioConfig.features[item.feature]),
    ...
    <aside className="bg-dark flex h-screen w-full flex-col md:fixed md:top-0 md:left-0 md:z-30 md:w-65">
      ...
      <div className="flex items-center gap-3 border-b border-white/10 px-5 py-5">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="truncate text-xs text-white/40">{studioConfig.studio.tagline}</p>
        </div>
        {onClose && (
          <button
            onClick={onClose}
