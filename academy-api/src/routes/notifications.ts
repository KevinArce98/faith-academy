...
notificationsRoutes.get('/', authMiddleware, async (c) => {
	...
	if (user.role === 'ADMIN' || user.role === 'TEACHER') {
		const [pendingOrders, expiringOrders, nearlyFullClasses] =
			await Promise.all([
		...
		const [pendingStudentOrders, rejectedOrders, expiringOrder] =
			await Promise.all([
				db.membershipOrder.findMany({
		...
		if (expiringOrder) {
			const daysLeft = Math.ceil(
				(expiringOrder.expiresAt!.getTime() - now.getTime()) /
					(1000 * 60 * 60 * 24),
			);
			notifications.push({
