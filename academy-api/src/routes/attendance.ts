...
attendanceRoutes.post('/attendance/scan', authMiddleware, async (c) => {
  ...
  try {
    const result = await db.$transaction(async (rawTx) => {
      ...
      if (!latestLedger || latestLedger.balance <= 0) throw new Error('NO_CREDITS');
      ...
      if (existingAttendance) {
        ...
        // Walk-in: find the current active class
    ...
    const knownReasons = ['MEMBERSHIP_INACTIVE', 'NO_CREDITS', 'NO_ACTIVE_CLASS'];
  ...
  try {
    const result = await db.$transaction(async (rawTx) => {
      ...
      const hoursUntilClass =
        (attendance.class.startsAt.getTime() - now.getTime()) / (1000 * 60 * 60);
      const withinWindow =
        hoursUntilClass >= 0 && hoursUntilClass >= attendance.class.cancelWindowHours;
    ...
    return c.json({ error: message }, (statusMap[message] ?? 500) as 403 | 404 | 409 | 500);
  }
});
