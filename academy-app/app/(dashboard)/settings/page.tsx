  ...
  const sections = [
    {
      icon: Building2,
      label: 'Estudio',
      desc: 'Nombre, logo, información general',
    },
    { icon: Palette, label: 'Apariencia', desc: 'Colores, tipografías, tema' },
    {
      icon: Bell,
      label: 'Notificaciones',
      desc: 'Correos, alertas, recordatorios',
    },
    { icon: Shield, label: 'Seguridad', desc: 'Roles, permisos, accesos' },
    {
      icon: Globe,
      label: 'Integraciones',
      desc: 'Pagos, redes sociales, APIs',
    },
    {
      icon: Settings,
      label: 'Modulos',
      desc: 'Feature flags y funcionalidades',
    },
  ];
        ...
        <div className="grid grid-cols-2 gap-3">
          {Object.entries(studioConfig.features).map(([key, enabled]) => (
            ...
            >
              ...
              <div
                className={`relative h-5 w-10 rounded-full transition-colors ${enabled ? 'bg-primary' : 'bg-gray-200'}`}
              >
                <div
                  className={`absolute top-0.5 h-3.5 w-3.5 rounded-full bg-white transition-transform ${enabled ? 'translate-x-5' : 'translate-x-0.5'} shadow`}
                />
              </div>
            </div>
          ))}
